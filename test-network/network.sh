...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Merkaba Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-merkaba.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Visa Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-visa.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.merkaba.cooperativa.com merkaba
    peer_cert admin Admin@merkaba.cooperativa.com merkaba

    infoln "Creating Visa Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.visa.cooperativa.com visa
    peer_cert admin Admin@visa.cooperativa.com visa

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.cooperativa.com
    orderer_cert admin Admin@cooperativa.com
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    echo "Entrando a CA"
    infoln "${CONTAINER_CLI_COMPOSE} -f compose/$COMPOSE_FILE_CA -f compose/$CONTAINER_CLI/${CONTAINER_CLI}-$COMPOSE_FILE_CA up -d"
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/merkaba/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating Socio1 Identities"
  ...
  # infoln "Generating CCP files for Merkaba and Visa"
...
function networkUp() {
  ...
  if [ ! -d "organizations/peerOrganizations" ]; then
    echo "Se van a generar las organizaciones (networkUp)"
    createOrgs
  fi
  ...
  infoln "$DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d"
...
# call the script to create the channel, join the peers of merkaba and visa,
# and then update the anchor peers for each organization
function createChannel() {
  infoln "Se va a levantar el canal $CHANNEL_NAME"
  ...
  infoln "scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true"
...
function networkDown() {
  ...
  # stop org3 containers also in addition to merkaba and visa, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.cooperativa.com docker_peer0.merkaba.cooperativa.com docker_peer0.visa.cooperativa.com docker_peer0.socio1.cooperativa.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/merkaba/msp organizations/fabric-ca/merkaba/tls-cert.pem organizations/fabric-ca/merkaba/ca-cert.pem organizations/fabric-ca/merkaba/IssuerPublicKey organizations/fabric-ca/merkaba/IssuerRevocationPublicKey organizations/fabric-ca/merkaba/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/visa/msp organizations/fabric-ca/visa/tls-cert.pem organizations/fabric-ca/visa/ca-cert.pem organizations/fabric-ca/visa/IssuerPublicKey organizations/fabric-ca/visa/IssuerRevocationPublicKey organizations/fabric-ca/visa/fabric-ca-server.db'
